...
\url{https://github.com/vsubhashini/ee380l-ghosh-project}
}
...
The project proposed by Professor Rai involves analyzing a set of approximately 1000 patents that pertain to solar energy technologies obtained from the US Patent and Trademark Office\cite{us_patent}. The information provided for each patent contains the following attributes:
...
Although these assumptions made by LDA aren't necessarily ideal and realistic to capturing the complete syntactic information, they are simple and efficient enough to help us model the hidden topics and thus infer the spillover assumptions.

The expected outcome of the project is to identify a language model (or a combination of models) and clustering techniques that can best capture knowledge transfer and spillovers. The end result is intended to be a general method that can find connections between patents regardless of type of technology as well as an analysis of the provided solar power patents.


\section{Data}
 The data for this project was initially a database of approximately 1000 entries containing 43 fields, including several forms of identification, classification, authors, owners, filing dates, and filing locations. The final analysis would use only 7 fields: patent number, filing year, city, state, country, tech area and CPC class. Additionally, the abstract and claims of the patent were added to the database after being taken from the USPTO website.
...
The primary challenges encountered were the previously mentioned hand-entered nature of the data and the nonuniform nature of the USPTO website. A significant amount of time was dedicated to preprocessing and authenticating this data to ensure accurate results. However, there were several mislabeled patents which could not be corrected. All of these that were located were removed, but it is likely that some remained undetected.  Additionally, recrawling the patents proved difficult because the USPTO’s website did not follow a standardized template and patents had inconsistent fields. To overcome this a large number of fringe cases needed to be written into the crawler.
...
\section{Methodology}
The methodology we decided to use consists of few carefully planned stages that put together eventually produced reasonable results for patent classification and analysis of knowledge spillover patterns in the field of solar energy. As mentioned previously we have focused primarily on applying method of clustering in our solutions. The main tool that we used throughout the course of this project is MALLET (Machine Learning for Language Toolkit) discussed in previous sections. The main features supplied by MALLET that we took advantage of are Topic Modelling and Classification. MALLET toolkit is an open source project developed by the University of Massachusetts Amherst.

Very soon into the project, it became clear that the best way of tackling the problem is through clustering. The nature of knowledge spillover is such that at the beginning there is not much data given on how the knowledge is being transferred. This ruled out classification or regression methods at least in the initial stage. We decided to choose MALLET as the toolkit package that we would be working with. It provides very good tools for textual analysis and is relatively easy in use and access, being written in Java and open source. Having wide range of features, MALLET would allow us to try different approaches to tackling the problem and to limit the need to use other outside tools.

The data we used was supplied to us by a research group for the knowledge spillover in solar energy field working under Dr Rai. The database we received was handpicked by this group and one of our major concerns was how to approach the handling these data. We ended up accessing United States Patent and Trademark Office (USPTO) website in order to obtain fuller and better representation of the data that we had initially received.

The above paragraphs show the most significant concerns and key decisions we had to make in regards to the methodology that we used throughout this project. After deciding these key factors we could concentrate on actual use of available resources in order to come up with a method to find out and analyze patterns in the knowledge spillover.

First, issue that we encountered and that we had to spend considerable amount of time on was data pre-processing. Cleaning up the data and deciding which parts of patent texts are the most suitable to use for the task at our hands turned out be quite tricky. We concluded that the best sections of patent documents to base spillover prediction on are abstracts and claims. Abstracts show the shortest and most concise understanding of what the patent is about and therefore seem to be perfect for our needs. However, abstracts are usually not long enough, in terms of text volume, to present on their own a considerable value in data mining applications. Therefore, abstracts needed to be supplemented. The best choice for the supplementary field turned out to be “claims” as they are the “heart” of the patent that in contrary to description or citations fields in general does not mention previous or other patents and topics. Patent’s citations often contain redundant entries that are presented only to protect the patent from lawsuits. Similarly, description often involves the whole historical and technical background of the patent and the inventor. This is not something that we are interested in. Therefore, we decided to use the two patent document fields in our project, namely, abstract and claims.

To obtain these fields, a web crawling script was created in Java that crawled USPTO’s website in order to get the required data fields. The script used patent number in case of patents and application number in case of applications to access the right patent or application webpage. At this point abstract and claims fields were extracted from the webpage in order to form the final entry that would be used from now on. The main difficulties with this step were different formats of application and patent numbers that did not fit in well with our script and adapting the script to work with these. Similarly, some patents had this number not provided, instead they had “publication number”, in these cases we had to get the right application/patent number first, before crawling. Also, the navigation of USPTO website search engines is fairly complicated so finding the right interfaces and getting accustomed to those took us awhile too.


After obtaining the data, the next step was to run topic modelling on the texts that we had obtained. The details of what is topic modelling have been provided in earlier paragraphs, so here we will just briefly state how topic modelling was applied to our dataset. The data for each patent, i.e. abstract and claims, has been put into a separate file. Later all these files were supplied to MALLET together with standard “stop words” list for English language list to perform the topic modelling on the data. When running topic modelling we tried to come up with different values of “n” – the number of topics created. After few runs and analysis of the outputs, we decided that “n” equal to 15 is the most reasonable option. Furthermore, since topic modelling in MALLET is not deterministic, we ran the modelling with n=15 few times to make sure that this is the right choice and to arrive with the best topics.

Following the topic modelling we assigned each topic a very general class that it represented by looking at the topic models themselves. The classes we used for this purpose were ``Solar Inverter'', ``Solar Mounting/Rack'', ``Solar Monitoring'' and ``Site Assessment''. An example of a topic model can be seen on Fig.~\ref{sample_topics} It is bag of words ordered by frequency of appearance in each topic. This topic model was ran for $n=15$. To assign a class to a topic we looked at the bag of words and arbitrarily decided which technology area each topic fits the most.
...
Next step was evaluating our results in attempt to see whether the topic models we arrived with are reasonable and reflect the actual patents distribution well. To approach this task we had to come up with a method that would compare our classes to some other method of classification. Since patent applications that arrive to USPTO are assigned classes by patent officers as they are filed we decided to use one of these as our method of evaluation. The patent classification scheme we decided to use is CPC – Cooperative Patent Classification. Each patent is assigned one or more CPC classes as its application is filed to USPTO and we used these classes to see whether our topic models constitute a valid solution.
\begin{figure}[h]
	\center
...
After ensuring that the topic models make sense we went on and grouped the data by geographies. Outside of the US it was done by continents. This is due to the fact that overwhelming number of patents, unsurprisingly, originated inside the US. The distribution of patents by countries can be seen on Fig.~\ref{geo_histogram} Inside the US, the clustering was done based on regions and states. We divided California into two regions, Bay Area and Southern California due to the great number of patents originating in that state. Following that we classified patents, using our topic models obtained earlier, into these topics. This step gave us an opportunity to see which topics are popular in which areas and to form conclusions on whether certain topics are more popular in given geographies implying knowledge sharing and spillover.

Similarly, we divided patents by years and subsequently classified them into topics to see in which years they were filed to see whether there are patterns of topics being more popular during certain time periods. This task was more difficult due to the fact that solar technology innovation field is relatively new and there is not many patents from the beginning of the time period (2006-2012) we were working on. On Fig.~\ref{year_histogram} we can see all patents grouped by year and technology area, before running topic modelling on them.
...
\section{Results}
We have run MALLET topic modelling on the solar energy patents data (claims and abstracts) carefully handpicked and crawled from the USPTO website. The objective was to determine whether data mining techniques could be used to predict and see knowledge spillover patterns in the field of solar power technology. The method we used consisted of running topic modelling on the patents then evaluating topic models by analysing whether they overlap with CPC classes to which each patent is assigned by the USPTO at the point of filing the patent application. Grouping the patents by geographies and years allowed us to see whether there appear to be any patterns in the development of patents that would imply knowledge spillover. This section of the paper is presenting the results that we have obtained.

The first question that is crucial to any further consideration of the results is whether we can use topic modelling to classify patents in the first place. This could be evaluated by computing whether patent topics obtained through topic modelling overlap with either technology area or/and CPC classes. There are 4 broad technology areas that patents cover: Solar Monitoring, Solar Mounting/Rack, Solar Inverter and Site Assessments. There are 23 CPC classes that patents composing our data represented. The overlap between the true class and modelled class obtained for topic modelling is presented in Table~\ref{results_table}. Fig.~\ref{ta_results_matrix} is a graphic representation of the confusion matrix of patents’ classification using topic modelling and technology areas.
...
The above data suggests that the classification of patents based on topic modelling is good and we can say with a significant degree of confidence that topic modelling can be used for the purpose of patent classification.

Next step is evaluating whether patents are associated with specific geographies and can topic modelling predict these associations. To answer these questions patents were divided into geographies and then classification based on topic models was performed on the patents. In Fig.~\ref{original_ta_geo} and Fig.~\ref{our_ta_geo} respectively we can see the most popular actual technology area and the most popular predicted technology area using topic modelling. Both seem to overlap almost completely and therefore, we can confidently give a positive answer to this problem.
...
As we observed above there are patents associated with specific geographies and topic modelling can identify that quite accurately. This result assures us that topic modelling is a valid method of classifying patents.
...
Another possible modification to our topic modelling would be to utilize a supervised topic modelling algorithm. These algorithms, such as the supervised s latent Dirichlet allocation algorithm (sLDA) \cite{blei2007} allow users to manually influence their topic modelling. This serves the purpose of modelling text more accurately for a specific purpose. For example, a standard topic model of the text of an entire patent may return several topics specific to patents but not the text area (such as the terms ìfigureî or ìtable). Through the use of a supervised method, the user can guide the topics towards terms such more specific to their goals. In our case these would be terms relevant to solar technologies.
